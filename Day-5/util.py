...
def get_sample_images(data_dir, dataset='mnist', n=25):
	"""
	...
	path_to_dataset = os.path.join(data_dir, dataset, '*.jpg')

	images = get_batch(glob.glob(path_to_dataset)[:n], 28, 28, mode)
	return images_square_grid(images, mode)

def get_single_image(data_dir, dataset='mnist'):
	...
	image = get_batch(glob.glob(path_to_dataset)[:1], 28, 28, mode)
	return image		
...
class Dataset(object):
	...
	def next_batch(self, batch_size):
		...
		new_epoch = False 
		IMAGE_MAX_VALUE = 255
		if self.current_index + batch_size > self.shape[0]:
			self.current_index = 0 # restart from 0
			new_epoch = True

		data_batch = get_batch(
			self.data_files[self.current_index:self.current_index + batch_size],
			*self.shape[1:3],
			self.image_mode)

		self.current_index += batch_size

		return new_epoch, data_batch / IMAGE_MAX_VALUE #- 0.5
